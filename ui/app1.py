...
# /* ═══ MIC FIXED ═══ — right side of chat bar */
# .mic-fixed {
...
# }
# .mic-fixed .stButton > button:hover {
...
#     right: 84px !important;         /* leave room for mic on the right */
...
#     word-break:break-word;max-width:calc(100% - 50px);
#     color: var(--text) !important;
# }
# .bubble.bot {
...
#                 st.markdown(f'<div class="file-card"><span class="file-ico">📄</span><span class="file-nm" title="{file}">{short}</span></div>', unsafe_allow_html=True)
#             with c2:
...
#                     st.rerun()
#     else:
#         st.markdown('<div class="no-files">📭 No documents yet<br><span style="font-size:12px">Upload a PDF to begin</span></div>', unsafe_allow_html=True)
...
#             delete_all_pdfs()
#             st.session_state.files = []; st.session_state.uploaded_files = []
#             st.toast("All PDFs removed", icon="🧹"); st.rerun()
#         st.markdown('</div>', unsafe_allow_html=True)
#     with c2:
#         st.markdown('<div class="btn-danger">', unsafe_allow_html=True)
...
#             clear_database()
#             st.session_state.files = []; st.session_state.uploaded_files = []
#             st.toast("Database reset", icon="⚠️"); st.rerun()
#         st.markdown('</div>', unsafe_allow_html=True)
...
# # ── Mic fixed bottom-right (always visible, right of chat input) ──
...
# # ── Chat input (CSS pins bottom, leaves right gap for mic) ──
...
# #         response = send_message(final_input)
...
#     thinking_id = len(st.session_state.messages)
